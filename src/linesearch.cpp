...
namespace STLBFGS
{
    ...
    double find_cubic_minimizer(double a, double fa, double ga, double b, double fb, double gb)
    {
        ...
        double D = z * z - ga * gb;
        if (D <= 0)
            return std::numeric_limits<double>::max(); // no minumum in the interval, +inf here because of the linesearch nature
    ...
    std::tuple<double, int> trial_value(const Sample &l, const Sample &t, const Sample &u, const bool bracketed)
    {
        ...
        { // Case 2: A lower function value and derivatives of opposite sign. The minimum is bracketed.
            double res = (std::abs(ac - t.a) >= std::abs(as - t.a)) ? ac : as;
            return std::make_tuple(res, 2);
        }

        if (std::abs(t.d) <= std::abs(l.d))
        {        // Case 3: A lower function value, derivatives of the same sign, and the magnitude of the derivative decreases.
            if ( // the cubic function may not have a minimizer; moreover, even if it exists, it can be in the wrong direction; fix it
                (l.a < u.a && ac <= t.a) ||
                (l.a > u.a && ac >= t.a))
                ac = u.a;
            double res = bracketed ? (std::abs(ac - t.a) < std::abs(as - t.a) ? ac : as) : (std::abs(ac - t.a) > std::abs(as - t.a) ? ac : as);
    ...
    bool line_search_more_thuente(const linesearch_function phi, const Sample phi0, double &at, const double mu, const double eta, const int lsmaxfev)
    {
        ...
        for (int nfev = 0; nfev < lsmaxfev; nfev++)
        {
            ...
            int caseno;
            std::tie(at, caseno) = stage1 && phit.f <= phil.f && psi(phit).f > 0 ? trial_value(psi(phil), psi(phit), psi(phiu), bracketed) : trial_value(phil, phit, phiu, bracketed);
            ...
            if (bracketed)
            {
                if (caseno == 1 || caseno == 3)
                {
                    ...
                    {                                                         // safeguard the trial value
                        ...
                        at = phil.a < phiu.a ? std::min(safeguard1, at) : std::max(safeguard1, at);
                        double safeguard2 = phil.a + .001 * (phiu.a - phil.a);
                        at = phil.a > phiu.a ? std::min(safeguard2, at) : std::max(safeguard2, at);
                    }
                }
                width_prev = width;
            }
    ...
    bool line_search_backtracking(const linesearch_function phi, const Sample phi0, double &at, const double mu, const double eta, const int lsmaxfev)
    {
        ...
        int nfev = 0;
        for (; nfev < lsmaxfev; nfev++)
        { // bracketing phase, see Numerical optimization, Algorithm 3.5 (Line Search Algorithm)
            phiu = phi(at);
            // TODO error handling
        ...
        for (; nfev < lsmaxfev; nfev++)
        { // zoom phase (bracketing succeeded), see Numerical optimization, Algorithm 3.6 (Zoom)
            if (
